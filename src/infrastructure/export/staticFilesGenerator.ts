 ...
 */
export function generateCredentialsTemplate(options?: { llmProviders?: string[] }): string {
...
openai:
  api_key: "\${OPENAI_API_KEY}"  # Set via environment variable or replace with your key`,
    anthropic: `
anthropic:
  api_key: "\${ANTHROPIC_API_KEY}"  # Set via environment variable or replace with your key`,
    cohere: `
cohere:
  api_key: "\${COHERE_API_KEY}"  # Set via environment variable or replace with your key`,
  };

  if (options?.llmProviders && options.llmProviders.length > 0) {
    const uniqueProviders = [...new Set(options.llmProviders)];
    const credBlocks = uniqueProviders
      .map((p) => providerCredentials[p])
      .filter(Boolean)
      .join('\n');

    return header + credBlocks + `
